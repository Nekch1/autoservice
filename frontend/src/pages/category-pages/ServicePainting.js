...
// const Painting = () =>{
//     return(
//         <CatalogPage categoryImg={categoryPainting} textContent={"Покраска кузова"}/>
//     )
// }
...
const CatalogPage = () => {
    ...
    useEffect(() => {
        fetch('http://localhost:5000/services/2')
            .then(res => res.json())
            .then(data => {
                console.log(data);
                setCategories(data);
                                ...
                                <div className="carousel-inner">
                                    {[0, 1, 2].map((i) => (
                                        <div key={i} className={`carousel-item ${i === 0 ? 'active' : ''}`}>
                                            <img src={categoryPainting} className="d-block w-100" alt={`Картинка ${i + 1}`} />
                                        </div>
                                    ))}
                                </div>
                            </div>
