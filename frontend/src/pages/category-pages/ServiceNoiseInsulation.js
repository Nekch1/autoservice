...
// const NoiseInsulation = () =>{
//     return(
//         <CatalogPage categoryImg={categoryNoiseInsulation} textContent={"Шумоизоляция"}/>
//     )
// }

// export default NoiseInsulation
...
const CatalogPage = () => {
    ...
    useEffect(() => {
        fetch('http://localhost:5000/services/8')
            .then(res => res.json())
            .then(data => {
                console.log(data);
                setCategories(data);
                                ...
                                <div className="carousel-inner">
                                    {[0, 1, 2].map((i) => (
                                        <div key={i} className={`carousel-item ${i === 0 ? 'active' : ''}`}>
                                            <img src={categoryNoiseInsulation} className="d-block w-100" alt={`Картинка ${i + 1}`} />
                                        </div>
                                    ))}
                                </div>
                            </div>
