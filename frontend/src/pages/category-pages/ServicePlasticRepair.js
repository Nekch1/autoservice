...
// const PlasticRepair = () =>{
//     return(
//         <CatalogPage categoryImg={categoryPlasticRepair} textContent={"Ремонт пластика"}/>
//     )
// }

// export default PlasticRepair
...
const CatalogPage = () => {
    ...
    useEffect(() => {
        fetch('http://localhost:5000/services/5')
            .then(res => res.json())
            .then(data => {
                console.log(data);
                setCategories(data);
                                ...
                                <div className="carousel-inner">
                                    {[0, 1, 2].map((i) => (
                                        <div key={i} className={`carousel-item ${i === 0 ? 'active' : ''}`}>
                                            <img src={categoryPlasticRepair} className="d-block w-100" alt={`Картинка ${i + 1}`} />
                                        </div>
                                    ))}
                                </div>
                            </div>
